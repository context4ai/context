...
For `submit_compile_cycle`, load the Node evidence via the returned command/views, Read `../skills/skill-compile-draft/SKILL.md` in full and follow it for exactly one Node, and pass the emitted JSON on stdin to the returned `next_action.command`.
