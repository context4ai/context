...
export class ProtoSyntaxError extends Error {
  constructor(message: string, readonly line: number, readonly column: number) {
    ...
    this.name = "ProtoSyntaxError";
  }
}

export function lexProto(source: string): ProtoToken[] {
  ...
  while (offset < source.length) {
    ...
    if (character === "/" && source[offset + 1] === "*") {
      ...
      if (!closed) throw new ProtoSyntaxError("unterminated block comment", startLine, startColumn);
      continue;
    }
    ...
    if (character === "\"" || character === "'") {
      ...
      if (!closed) throw new ProtoSyntaxError("unterminated string", tokenLine, tokenColumn);
    ...
    throw new ProtoSyntaxError(`unsupported character ${JSON.stringify(character)}`, line, column);
  }
  return tokens;
}
