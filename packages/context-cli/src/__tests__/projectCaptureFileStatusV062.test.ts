...
import {
  ...
  readRunLogs,
  runCliInDir,
  writeAliasedAlignProjectEntry,
  writeAmbiguousAlignProjectEntry,
  writeCaptureProjectEntry,
} from "./projectCaptureFileV062Helpers.js";

describe("0.6.2 capture:file status and refresh", () => {
  test("capture:file reevaluates the workspace and status resolves registry aliases", async () => {
    const root = makeTmp();
    try {
      ...
      writeFileSync(join(projectRoot, "sources", "file", "index.yaml"), [
        "sources:",
        "  - id: product-docs-alias",
        "    name: product-docs",
        "    local: ../docs",
        "",
      ].join("\n"), "utf8");
      writeAliasedAlignProjectEntry(projectRoot);
      ...
      expect(output.result.source.name).toBe("product-docs");
      expect(output.result.next_action).toMatchObject({
      ...
      const status = JSON.parse(await runCliInDir(projectRoot, [
        ...
        workflow: { current: { node: string; reason_code: string } };
      ...
      expect(status.pendingStructureTargets).toContainEqual(expect.objectContaining({
        ...
        alignPhaseId: "align:source:product-docs-alias:architecture",
        command: "context run align:source:product-docs-alias:architecture --view read-plan --format json",
        configurationGaps: ["compile", "review"],
      }));
    } finally {
  ...
  test("capture:file lets status expose every configured collection without choosing one", async () => {
    const root = makeTmp();
    try {
      const result = await initContextProject({ cwd: root, projectDir: "kb", dev: true });
      const projectRoot = result.projectRoot;
      const docsDir = join(projectRoot, "..", "docs-ambiguous-align");
      ...
      writeAmbiguousAlignProjectEntry(projectRoot);
      ...
      expect(status.pendingStructureTargets.find((target) => target.collection === "architecture")).toMatchObject({
        sourceKey: "file:product-docs",
        command: "context run align:source:product-docs:architecture --view read-plan --format json",
        configurationGaps: ["compile", "review"],
      });
      expect(status.pendingStructureTargets.find((target) => target.collection === "product")).toMatchObject({
        sourceKey: "file:product-docs",
        command: "context run align:source:product-docs:product --view read-plan --format json",
        configurationGaps: ["compile", "review"],
      });
    } finally {
      rmSync(root, { recursive: true, force: true });
