...
test("an accepted contract page plan and its content retry both produce one valid primary bundle", async () => {
  ...
  try {
    ...
    const input = indexerAuthorSemanticInputSchema.parse({ stage: "author", group_key: workset.group_key, outcome: "publish", artifact_intent: primaryIntentKey(contract), policy: "standard", title: "Public interface", summary: "Reference for the public entry.", sections: [{ key: "overview", heading: "Overview", markdown: "Use the exported entry.", references, answers: validation.allowed_question_targets.map(target => target.question_target_key) }], member_dispositions: validation.canonical_inventory_members.map(member => ({item: member.member_id, state: "covered", section: "overview"})), material_gaps: [], diagnostics: [] });
    const build = (artifact_intent: string) => buildIndexerAuthorRunResultFromSemantic({ projectRoot: root, request: task.spec.request, view: task.view, validation: planned, semantic: { ...input, artifact_intent } });
    expect(() => buildIndexerAuthorRunResultFromSemantic({ projectRoot: root, request: task.spec.request, view: task.view,
      validation: planned, semantic: { ...input, sections: [{ ...input.sections[0]!, references: [{ ...references[0]!, source_ref: "repo:unavailable" }] }] } })).toThrow();
