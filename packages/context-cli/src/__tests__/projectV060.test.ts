...
describe("0.6.0 project init and source ensure", () => {
  test("context init creates a project-local skeleton in the default context directory", async () => {
    ...
    try {
      ...
      expect(agents).toContain("## Workspace Contract");
      expect(agents).toContain("## Manuals");
      ...
      expect(agents).toContain("When the Route returns `configuration`, edit only the named project files");
  ...
  test("context init uses an explicit language for workspace and package starter templates", async () => {
    ...
    try {
      ...
      expect(agents).toContain("## 工作区契约");
      expect(agents).toContain("将 `workflow.current` 及其选择的资源和命令视为当前步骤的唯一权威");
      expect(agents).toContain("所有生命周期写入必须使用 Context CLI");
      expect(agents).toContain("Context 完成只证明知识工作流状态，不证明 Git 提交范围安全");
      expect(agents).toContain("包模板：`node_modules/@c4a/context/docs/reference/package-templates.md`");
      expect(agents).not.toContain("全托管");
      expect(agents).not.toContain("execution.target");
