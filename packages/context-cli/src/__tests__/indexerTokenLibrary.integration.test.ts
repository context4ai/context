...
test("a token-only library can use an explicit system subject without React exports", async () => {
  ...
  try {
    await completePartitionStage(root, true, false, "theme-tokens", undefined, "component-library-l03");
    const review = (await currentIndexerStructureReview(root))!;
    expect(JSON.stringify(review.preview)).toContain("theme-tokens");
    expect(JSON.stringify(review.preview)).not.toContain("subject_kind");
