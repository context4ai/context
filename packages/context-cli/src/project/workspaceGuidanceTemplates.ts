...
export function renderAgents(projectName: string, language: ProjectLanguage): string {
  ...
  return [
    ...
    "- Broad document optimization is reachable only when `package.json.context.documentOptimization=true`. When the user asks to correct existing knowledge, run `context revise \"<title, approved path, or ViewRef>\" --format json`, then follow `route.document-revision.requested` to edit the sibling `knowledge/**/*__revision.md` page and validate it; never use revisions to change facts.",
