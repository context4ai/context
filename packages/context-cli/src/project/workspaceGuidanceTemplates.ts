...
export function renderProjectEntry(language: ProjectLanguage): string {
  if (language === "zh-CN") {
    return [
      ...
      "",
      "// 普通来源用 `context source add ... --configure` 自动生成显式采集配置；特殊配置在此修改。",
      "// 同类来源配置优先复用登记表＋map；只选择任务范围内来源，参考 SDK 文档的 Batch capture。",
  ...
  return [
    'import { defineProject } from "@c4a/context";',
    "",
    "// Use `context source add ... --configure` for explicit default capture configuration; customize here.",
    "// Reuse registry entries + map for shared capture settings; select only in-scope sources (SDK: Batch capture).",
...
export function renderReadme(projectName: string, language: ProjectLanguage): string {
  if (language === "zh-CN") {
    return [
      ...
      "- `.tmp/context-runtime/`：Context 运行时状态。成功交付后由 CLI 清理已完成任务；不要手动清空运行时目录。",
      "",
  ...
  return [
    ...
    "- `.tmp/context-runtime/`: Context runtime state. The CLI removes completed task state after successful delivery; do not manually empty the runtime directory.",
    "",
...
export function renderAgents(projectName: string, language: ProjectLanguage): string {
  if (language === "zh-CN") {
    return [
      ...
      "版本差异使用 Git 比较正文、结构、资源、导航和模板；可通过 version inspect --base 指定交付 commit/tag。构建预览不锁定版本；复用版本前核对目标平台与 Git tag，已发布版本必须递增。无需维护根目录 Context 版本、构建或发布回执。",
  ...
  return [
    ...
    "Compare bodies, structure, assets, navigation and templates using Git; version inspect --base selects a delivery commit/tag. Preview builds do not seal versions. Check the remote target and Git tags before amending; published versions must increase. No root Context version/build/publication receipts are required.",
