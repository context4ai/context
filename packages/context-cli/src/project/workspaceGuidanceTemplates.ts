...
export function renderAgents(projectName: string, language: ProjectLanguage): string {
  if (language === "zh-CN") {
    return [
      ...
      "- 用户要求修正当前候选时，运行 `context revise \"<候选标题、路径或 id>\" --instruction \"<修正要求>\" --format json`，让 Context 精确重开其 Author workset；不得直接编辑 `knowledge/` 或创建旁路 revision 页面。",
  ...
  return [
    ...
    "- When the user asks to correct a current Candidate, run `context revise \"<candidate title, path, or id>\" --instruction \"<requested correction>\" --format json` so Context reopens exactly the owning Author workset. Never edit `knowledge/` directly or create a side-channel revision page.",
