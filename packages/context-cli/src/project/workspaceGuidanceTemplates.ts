...
export function renderReadme(projectName: string, language: ProjectLanguage): string {
  ...
  return [
    ...
    "- `knowledge/`: durable knowledge, its structural projection with minimal closed source inputs, compact rejected-candidate fingerprints, and optional `*__revision.md` document revision sidecars excluded from default knowledge discovery.",
...
export function renderAgents(projectName: string, language: ProjectLanguage): string {
  if (language === "zh-CN") {
    return [
      ...
      "- 整库文档编译优化只在 `package.json.context.documentOptimization=true` 时可达。用户要求修正已有知识时，先运行 `context revise \"<标题、正式路径或 ViewRef>\" --format json`，再按 `route.document-revision.requested` 编辑紧邻原文的 `knowledge/**/*__revision.md` 并校验；不得用它改写事实。",
      "- 普通 Review 使用用户原样提供的 Payload；首次展示审核时不要主动介绍逃生路径。仅当用户无法使用报告并在当前会话明确回复 `强制批准` 时，才执行 Route 返回的 revision-bound 强制批准命令。托管批准仍只使用托管 status 返回的原子命令。",
  ...
  return [
    ...
    "- Source-constrained editorial revision is reachable only when `package.json.context.documentOptimization=true`. Follow the Route plan to keep, repair, reshape, or safely omit content inside its existing source Section; preserve facts and protected values. When the user asks to correct existing knowledge, run `context revise \"<title, approved path, or ViewRef>\" --format json`, then follow `route.document-revision.requested` to edit the sibling `knowledge/**/*__revision.md` page and validate it; never use revisions to change facts.",
    "- Review uses the user's exact Payload in ordinary mode, and the initial prompt must not advertise the escape path. Only when the user cannot access the report and replies with the exact phrase `强制批准` in the current conversation may the Agent execute the Route-returned revision-bound force-approval command. Managed approval still uses only the atomic command returned by managed status.",
