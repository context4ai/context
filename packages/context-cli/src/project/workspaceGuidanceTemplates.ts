...
export function renderAgents(projectName: string, language: ProjectLanguage): string {
  if (language === "zh-CN") {
    return [
      ...
      "首次成功构建或发布前的正式修补，如 version inspect 返回 reusable_version，复用该版本并更新完整的本轮 changelog（原始工作＋修补）；成功构建或发布后不再复用。不得删除回执来改写历史。",
  ...
  return [
    ...
    "For formal repairs before the first successful build or publication, reuse reusable_version from version inspect when present and replace the pending entry with the complete iteration description, including original work and repairs. Successful build/publication seals the version. Never delete receipts to reopen history.",
