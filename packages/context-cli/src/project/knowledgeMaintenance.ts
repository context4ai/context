...
async function deliveryDigest(root: string) {
  const directory = join(root, ".tmp/context-runtime/packages");
  try {
    const files = (await readdir(directory)).filter(name => name.endsWith(".json")).sort();
    if (!files.length) return indexerProtocolDigest(null);
    return indexerProtocolDigest(await Promise.all(files.map(name => readFile(join(directory, name), "utf8"))));
