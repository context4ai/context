...
export async function productionWorkflowRoute(input: {
  ...
  return { protocol: "context.workflow.route.v1", id: resolved.routeId, node: resolved.node,
    ...
    summary: report ? `Present the report and apply context.gate.work_start_scope. After the applicable scope decision, write {stage: ${stage.id}, decision: approved} to ${path}.`
      ...
      : investigate ? `Continue this request's pending investigation: ${stage.pending_scopes.filter(scope => !stage.gaps.some(gap => gap.scope === scope)).length} sources (details in planning.md). Submit additional article tasks and the actual remaining investigation; preserve unfinished requested work. Sources authorized in the long-term requirements may be added as article dependencies when needed. For an explicitly requested partial delivery, context run --deliver --format json reviews completed articles while retaining the remaining work.`
      : resolved.node === "resolve-production-gap" ? `Source availability gaps: ${stage.gaps.length} selected sources (details in planning.md). These selected sources are needed for remaining investigation or article dependencies. Preserve completed articles and restore the required material. To review completed articles independently, use context run --deliver --format json; this retains pending scopes and does not approve or publish content. If these sources are required, use context source recovery-plan --format json and context source restore with explicit local or clone decisions before preparation.` : "Prepare the current stage's eligible task directories.",
    commands: [...(report || prepare || writing || repair || investigate || gap ? [{ command, effect: "write" as const, availability: report || gap ? "after-human-confirmation" as const : "immediate" as const,
      managed_execution: prepare ? "automatic" as const : "agent-required" as const }] : []),
      ...((investigate || gap) && stage.tasks.some(task => task.status === "accepted") ? [{
        command: "context run --deliver --format json", effect: "write" as const,
        availability: "after-human-confirmation" as const, managed_execution: "agent-required" as const,
      }] : [])],
