...
export const DOCUMENT_OPTIMIZATION_ROOT = join("overlays", "document-optimization");
export const DOCUMENT_OPTIMIZATION_POLICY = "context.document-optimization.v1";
...
export async function enableDocumentOptimization(projectRoot: string): Promise<void> {
  ...
  await mkdir(join(documentOptimizationRoot(projectRoot), "generated", "views"), { recursive: true });
  await mkdir(join(documentOptimizationRoot(projectRoot), "generated", "fragments"), { recursive: true });
  await mkdir(join(documentOptimizationRoot(projectRoot), "overrides"), { recursive: true });
}
...
export async function disableDocumentOptimization(projectRoot: string): Promise<string | undefined> {
  ...
  if (existsSync(overlayRoot)) {
    ...
    recoveryPath = join(
      recoveryRoot,
      `${basename(overlayRoot)}-${recoveryTimestamp()}`,
    );
    await mkdir(dirname(recoveryPath), { recursive: true });
    await rename(overlayRoot, recoveryPath);
  }
