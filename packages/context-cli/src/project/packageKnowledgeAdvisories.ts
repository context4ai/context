...
export async function withPackageKnowledgeAdvisories(projectRoot: string, files: readonly ApprovedKnowledgeFile[]): Promise<ApprovedKnowledgeFile[]> {
  const stale = new Set((await approvedKnowledgeDependencyWarnings(projectRoot))
    .filter(issue => issue.code === "approved-source-region-changed").map(issue => issue.path));
  return files.map(file => {
    if (!stale.has(file.relPath)) return file;
    const notice = "> **Source regions need review.** A cited source region changed or is unavailable. Recheck the linked sources before relying on this article.\n\n";
