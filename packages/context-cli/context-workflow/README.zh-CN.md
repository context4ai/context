...
17. Indexer Graph 通过 CLI 校验后的 workset-set Facts 从 partition 进入 author，不要求先建模主体；post-author composer 读取受限 PrimaryResultView。独立 composer 集合未明确标记为 not-required，或尚未全部 accepted 并形成 current envelope 时，不进入 reconciliation。
...
25. 稳定文章与片段身份用于跨修订定位正文。来源引用记录实际路径和行号，由 Host 计算区域指纹；主体目录、关系矩阵和文章事实账本不再是生产前提。
