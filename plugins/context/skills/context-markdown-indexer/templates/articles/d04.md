...
按读者任务综合相关文档，不默认每份原文生成一页。给出本主题可直接回答的内容、
必须回原文或实时系统核实的条件，以及“问题 → 对应主题／原文”的导航。
把抓取成功、正文可读、已实际综合区分开；空正文和无权限只保留入口与限制。
历史方案不写成当前行为，多个来源冲突时注明适用版本和待核项。
长篇先提供主题定位，小段落可以合并；不要为了凑篇数拆成重复概览。
