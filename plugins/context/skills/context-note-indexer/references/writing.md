...
A note can contain an observation, excerpt, proposal or an explicitly confirmed
decision. Judge the actual record, not its source type: retain a documented
approval with its attribution and scope, without claiming to have read an
unavailable original. Do not promote an unconfirmed proposal to policy or demote
an explicit confirmation merely because it is stored as a note.

State dates, versions and environments when they affect how the answer can be
used. If a missing boundary matters and authorized material does not settle it,
qualify that claim locally; do not fill every page with generic unknowns. Do not
substitute the note's save date for the event date, or treat the absence of a
recorded counter-example as proof that one observation generalizes.
