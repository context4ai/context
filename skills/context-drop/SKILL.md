...
name: context-drop
description: >
  Drop a source: CLI plans cleanup, reconciles affected Sections when needed, then applies safely. Equivalent to Claude /context:drop; use the local `context` CLI for workspace writes.
tools:
  - Bash
---
...
- Public entry: `context-drop`
- Claude equivalent: `/context:drop`
...
Read `references/internal-procedures/skill-drop.md` in full and follow it end to end, including the self-verify checklist at the end.
