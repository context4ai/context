...
description: >
  Capture URLs, local Markdown, source code, stdin path lists, inbox/refresh sources, or conversation notes as Context sources. Equivalent to Claude /context:capture; use the local `context` CLI for workspace writes.
tools:
  - Bash
---
...
- Public entry: `context-capture`
- Claude equivalent: `/context:capture`
